...
const headCells = [
  ...
  { id: 'email', label: 'Email', numeric: false, disablePadding: true, width: 150 },
  { id: 'role', label: 'Role', numeric: false, disablePadding: true, width: 150 },
];
...
const DataView = () => {
  ...
  const [url, setUrl] = useState(`${API_URL}/users?page=${page}&rowsPerPage=${rowsPerPage}&search=${searchPath}`);
  ...
  const handleChangePage = (event, newPage) => {
    setRows([]);
    setUrl(`${API_URL}/users?page=${newPage+1}&rowsPerPage=${rowsPerPage}&search=${searchPath}`);
  };

  const handleChangeRowsPerPage = (event) => {
    setRows([]);
    setUrl(`${API_URL}/users?page=1&rowsPerPage=${parseInt(event.target.value, 10)}&search=${searchPath}`);
  };
  ...
  const handleOnSearch = (event) => {
    ...
    setUrl(`${API_URL}/users?page=1&rowsPerPage=${rowsPerPage}&search=${path}`) 
    : setUrl(`${API_URL}/users?page=${page}&rowsPerPage=${rowsPerPage}&search=${path}`);
  };
